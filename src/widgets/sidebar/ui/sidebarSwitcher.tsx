"use client";

import { Button } from "@/shared/ui/button/button";

import { useSidebar } from "../model/sidebar.store";

import cls from "./sidebarSwitcher.module.css";

export const SidebarSwitcher = (): JSX.Element => {
    const { setStatus } = useSidebar();

    return (
        <Button theme="clear" onClick={() => setStatus()}>
            <svg viewBox="0 0 32 32" width="30" height="30" className={cls.burgerIcon}>
                <path
                    d="M4,10h24c1.104,0,2-0.896,2-2s-0.896-2-2-2H4C2.896,6,2,6.896,2,8S2.896,10,4,10z M28,14H4c-1.104,0-2,0.896-2,2s0.896,2,2,2h24c1.104,0,2-0.896,2-2S29.104,14,28,14z M28,22H4c-1.104,0-2,0.896-2,2s0.896,2,2,2h24c1.104,0,2-0.896,2-2S29.104,22,28,22z"
                    fill="#909090"
                />
            </svg>
        </Button>
    );
};
